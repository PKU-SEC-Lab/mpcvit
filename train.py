...
parser.add_argument('--search-ckpt', type=str, default='',
                    help='search checkpoint')
...
def main():
    ...
    try:
        # sticker: train and validate the model in every epoch
        ...
        ## remove unimportant ReLUSoftmax and directly inference
        if not args.retrain_mode:
            print('It is not in the retraining mode now!')
            return 
        print('The mode of retraining!')
        # model_path = './mpcvit_cifar10-0.7.pth.tar'
        model_path = args.search_ckpt
        ...
        print('Ratio of ReLUSoftmax:', rs_ratio)
        ...
        print('Inferece after directly alpha binarization...')
        eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)

        # after removing unimportant ReLUSoftmax, we need to retrain (with knowledge distillation) for more epochs
        if args.retrain_mode:
        ...
        if args.use_kd or args.use_token_kd:
            # KD: vit with the original softmax ViT
            model_teacher = create_model(
                ...
                scriptable=args.torchscript,
                checkpoint_path='')
            # teacher_path = './output/train/cifar100-vit_7_4_32-32-origin(600epochs)/model_best.pth.tar'
            teacher_path = args.teacher_ckpt
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.search_mode:
                ...
                loss = loss_fn(output, target)  # baseline
